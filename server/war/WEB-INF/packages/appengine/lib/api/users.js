...
var JHashSet = Packages.java.util.HashSet,
    JUserServiceFactory = Packages.com.google.appengine.api.users.UserServiceFactory,
    JUser = Packages.com.google.appengine.api.users.User,
    jservice = JUserServiceFactory.getUserService();

var endsWith = require("ringo/utils/strings").endsWith;

var defineError = require("appengine/utils").defineError;
 ...
 */
var User = exports.User = function (email, authDomain, userId, federatedIdentity, federatedProvider) {
    if (email) {
        this.email = email;
 ...
 * @param {string} authDomain Ignored.
 ...
 */
exports.createLoginURL = function (options) {
    if (typeof(options) == "string") {
        return jservice.createLoginURL(/* destURL */options);
    } else {
        if (options.federatedIdentity) {
            return jservice.createLoginURL(options.destURL, options.authDomain, options.federatedIdentity, new JHashSet());
        } else {
            return jservice.createLoginURL(options.destURL);
        }        
    }
};
...
exports.isUserLoggedIn = function(){
	return jservice.isUserLoggedIn();
};
